...
Re-running is safe — both `opflow project update` and `opflow docs write` are idempotent upserts. Don't re-run unprompted.

## CLI commands used

- `opflow project get --json` — read current project (name, purpose, technologies, monorepo, githubrepo, bootstrapDocId).
- `opflow project update --technologies @tech.md` — set `technologies` (markdown). Owner-only mutation.
- `opflow docs write --path bootstrap.md --title Bootstrap --package root --tags '["bootstrap"]' --content @bootstrap.md` — replace the body of `bootstrap.md`.
- `opflow search` / `opflow context` / `opflow files` — explore the indexed codebase to ground the writeup.

## Workflow

1. **Read current state.** Call `opflow project get --json`. Note `name`, `purpose`, `monorepo`, and whether `technologies` is empty.
2. **Survey the code.** Use `opflow files` to enumerate top-level packages / entry points. Use `opflow search` for distinctive markers (framework imports, build configs, test runners, deployment specs). Don't read every file — sample enough to characterize the stack.
...
5. **Write.** Run `opflow project update --technologies @tech.md`, then `opflow docs write --path bootstrap.md --title Bootstrap --package root --tags '["bootstrap"]' --content @bootstrap.md`.
6. **Confirm.** Echo the two lengths back to the user (e.g. "Wrote 480 chars to technologies, 1.2 KB to bootstrap.md") and stop.

## Constraints

- **Owner only:** `opflow project update` rejects non-owner callers. If it 403s, surface the error and stop — don't silently retry as a different field.
- **Don't invent project purpose.** If the existing `purpose` field is empty and you can't infer one confidently from the code, leave it untouched — propose an edit to the user instead of writing it via `opflow project update`.
